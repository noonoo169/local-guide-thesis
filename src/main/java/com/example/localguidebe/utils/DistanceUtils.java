...
public class DistanceUtils {
  public static double calculateDistanceVincenty(
    ...
    do {
      sinSigma =
          Math.sqrt(
              Math.pow(cosU2 * Math.sin(longitudeDifference), 2)
    ...
    double uSq =
        cosSqAlpha
            * (Math.pow(SEMI_MAJOR_AXIS_MT, 2) - Math.pow(SEMI_MINOR_AXIS_MT, 2))
  ...
  public static Double getAvgDistance(List<Double> distances) {
    return distances.stream().mapToDouble((distance) -> distance).summaryStatistics().getAverage();
  }
}
